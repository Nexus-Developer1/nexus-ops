<div>
    <x-topbar :breadcrumb="['Início', 'Clientes', $cliente->nome, 'Contratos']">
    ...
    <main class="flex-1 px-4 py-6 sm:px-10 sm:py-9">
        <div class="mx-auto max-w-6xl">
            ...
            <p class="mt-2 text-sm text-texto-medio">{{ $cliente->nome }} · {{ $contratos->total() }} {{ \Illuminate\Support\Str::plural('contrato', $contratos->total()) }}</p>
                ...
                <div class="overflow-x-auto"><table class="w-full min-w-[640px] text-left text-sm">
                    ...
                    <tbody>
                        @forelse ($contratos as $ct)
                            <tr class="border-b border-borda transition last:border-0 hover:bg-fundo" wire:key="ct-{{ $ct->id }}">
                                <td class="px-6 py-4 font-medium text-texto-forte">{{ $ct->numero }}</td>
                                <td class="px-6 py-4 text-texto-medio">{{ $ct->modeloFaturacao?->nome ?? '—' }}</td>
                                <td class="px-6 py-4 text-texto-medio">{{ $ct->data_inicio?->translatedFormat('d M Y') ?? '—' }} – {{ $ct->data_fim?->translatedFormat('d M Y') ?? '—' }}</td>
                                <td class="px-6 py-4"><span class="etiqueta {{ $ct->estado->classesEtiqueta() }}">{{ $ct->estado->rotulo() }}</span></td>
                            </tr>
                        @empty
                            <tr><td colspan="4" class="px-6 py-12 text-center text-sm text-texto-medio">Sem contratos.</td></tr>
                        @endforelse
                    </tbody>
                </table></div>
            </div>
