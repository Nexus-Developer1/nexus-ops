                    ...
                    <tbody>
                        @forelse ($equipamentos as $eq)
                            <tr class="border-b border-borda transition last:border-0 hover:bg-fundo" wire:key="eq-{{ $eq->id }}">
                                <td class="px-6 py-4">
                                    <div class="font-medium text-texto-forte">{{ trim($eq->fabricante . ' ' . $eq->modelo) ?: '—' }}</div>
                                    <div class="text-xs text-texto-fraco">{{ $eq->tipo->rotulo() }}</div>
                                </td>
                        ...
                        @empty
                            <tr><td colspan="4" class="px-6 py-12 text-center text-sm text-texto-medio">Sem equipamentos associados.</td></tr>
                        @endforelse
                    </tbody>
                </table></div>
            </div>
