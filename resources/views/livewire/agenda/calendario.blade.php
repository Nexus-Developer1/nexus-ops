                    ...
                    <form wire:submit="criarEvento" class="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-xl border border-borda bg-white shadow-xl">
                        ...
                        <div class="space-y-5 px-6 py-5">
                            ...
                            {{-- Cliente: combobox server-side (nome/NIF/nº ERP). Restringe a pesquisa de
                                 equipamentos; escolher um equipamento preenche-o. Chip com × (trancado num
                                 evento já convertido — o cliente é do relatório). --}}
                            <div>
                                <label class="campo-label" for="cliente-evento-combo">Cliente</label>
                                <div x-data="{ aberto: false, destaque: 0 }" @click.outside="aberto = false" @keydown.escape.stop="aberto = false" class="relative">
                                    <input
                                        id="cliente-evento-combo"
                                        type="text"
                                        wire:model.live.debounce.300ms="formClienteBusca"
                                        @disabled($editandoConvertido)
                                        @focus="aberto = true"
                                        @click="aberto = true"
                                        @input="aberto = true; destaque = 0"
                                        @keydown.arrow-down.prevent="aberto = true; if ($refs['copt' + (destaque + 1)]) destaque++"
                                        @keydown.arrow-up.prevent="if (destaque > 0) destaque--"
                                        @keydown.enter.prevent="$refs['copt' + destaque]?.click()"
                                        class="campo-input pr-10 disabled:bg-fundo disabled:text-texto-fraco"
                                    ...
                                    <ul x-show="aberto && !{{ $editandoConvertido ? 'true' : 'false' }}" x-cloak x-transition.opacity class="absolute z-20 mt-1 max-h-60 w-full overflow-auto rounded-lg border border-borda bg-white py-1 shadow-lg" role="listbox">
                                        @forelse ($clientesFiltrados as $idx => $cl)
                                            <li x-ref="copt{{ $idx }}" wire:key="cl-ev-{{ $cl->id }}"
                                        ...
                                        @empty
                                            <li class="px-4 py-2 text-sm text-texto-medio">
                                                {{ $formClienteBusca === '' ? 'Escreva para pesquisar…' : 'Nenhum cliente encontrado.' }}
                                            </li>
                                        @endforelse
                                    </ul>
                                </div>
                                @error('formClienteId') <p class="mt-1.5 text-xs text-perigo-500">{{ $message }}</p> @enderror
                                @if ($clienteEscolhido)
                                    <div class="mt-2 flex flex-wrap gap-2">
                                        <span wire:key="chip-cl-{{ $clienteEscolhido->id }}" class="inline-flex items-center gap-1.5 rounded-full border border-verde-200 bg-verde-50 px-3 py-1 text-xs font-medium text-verde-700">
                                            {{ $clienteEscolhido->nome }}
                                            @if ($clienteEscolhido->id_erp)<span class="font-normal text-verde-600">· Nº {{ $clienteEscolhido->id_erp }}</span>@endif
                                            @unless ($editandoConvertido)
                                                <button type="button" wire:click="removerCliente" class="ml-0.5 rounded-full text-verde-600 hover:text-perigo-600" title="Tirar" aria-label="Tirar {{ $clienteEscolhido->nome }}">&times;</button>
                                            @endunless
                                        </span>
                                    </div>
                                @endif
                            ...
                            <div>
                                ...
                                <div x-data="{ aberto: false, destaque: 0 }" @click.outside="aberto = false" @keydown.escape.stop="aberto = false" class="relative">
                                    ...
                                    <ul x-show="aberto" x-cloak x-transition.opacity class="absolute z-20 mt-1 max-h-60 w-full overflow-auto rounded-lg border border-borda bg-white py-1 shadow-lg" role="listbox">
                                        ...
                                        @empty
                                            <li class="px-4 py-2 text-sm text-texto-medio">
                                                {{ $formEquipamentoBusca === '' && ! $formClienteId ? 'Escreva para pesquisar…' : 'Nenhum equipamento encontrado.' }}
                                            </li>
                                        @endforelse
                                    </ul>
                                </div>
                            ...
                            <div>
                                ...
                                <label class="mt-3 flex cursor-pointer items-center gap-3 rounded-lg border border-borda px-4 py-3 text-sm text-texto-forte">
                                    ...
                                    <span>Avisar os técnicos por email <span class="block text-xs font-normal text-texto-fraco">ao criar, alterar ou remover este evento (todos os técnicos associados, incluindo quem faz a ação)</span></span>
                                </label>
