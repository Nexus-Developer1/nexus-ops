...
class AlertasAtribuicaoTest extends TestCase
{
    ...
    public function test_alerta_de_equipamento_atribuido_so_aparece_a_essa_pessoa_e_aos_admins(): void
    {
        ...
        $titulos = fn (User $u) => $this->servico()->recolher()->filter(fn ($a) => ServicoAlertas::visivelPara($a, $u))->pluck('titulo')->all();
    ...
    public function test_alerta_de_evento_vai_aos_tecnicos_do_evento_principal_e_adicionais(): void
    {
        ...
        $this->assertCount(1, $this->servico()->recolher()->filter(fn ($a) => ServicoAlertas::visivelPara($a, $this->rui))->where('tipo', 'evento_programado'));
        $this->assertCount(1, $this->servico()->recolher()->filter(fn ($a) => ServicoAlertas::visivelPara($a, $this->julio))->where('tipo', 'evento_programado'));
        $this->assertCount(0, $this->servico()->recolher()->filter(fn ($a) => ServicoAlertas::visivelPara($a, $ana))->where('tipo', 'evento_programado'));
    }
    ...
    public function test_alerta_de_despesa_vai_aos_aprovadores_com_conta(): void
    {
        ...
        $this->assertCount(1, $this->servico()->recolher()->filter(fn ($a) => ServicoAlertas::visivelPara($a, $paulo))->where('tipo', 'despesa_aprovacao'));
        $this->assertCount(0, $this->servico()->recolher()->filter(fn ($a) => ServicoAlertas::visivelPara($a, $this->rui))->where('tipo', 'despesa_aprovacao'));
        $this->assertCount(1, $this->servico()->recolher()->filter(fn ($a) => ServicoAlertas::visivelPara($a, $this->admin))->where('tipo', 'despesa_aprovacao'));
    }
    ...
    public function test_dashboard_e_painel_mostram_a_cada_um_o_que_lhe_cabe(): void
    {
        ...
        // Dashboard: TODOS veem tudo (mesmos números da página de Alertas); o nome de quem
        // trata aparece em cada alerta.
        Livewire::actingAs($this->rui)->test(DashboardGestao::class)
            ->assertSee('SO-RUI')->assertSee('EQUIPA-TODA')->assertSee('SO-JULIO');
        ...
        // Painel: por defeito "todos" (bate com o dashboard); "os meus" filtra.
        Livewire::actingAs($this->julio)->test(Painel::class)
            ->assertSee('SO-JULIO')->assertSee('EQUIPA-TODA')->assertSee('SO-RUI')
